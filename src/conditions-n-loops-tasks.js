...
function getSpiralMatrix(size) {
  ...
  for (let i = 0; i < size ** 2; i += 1) {
    arr[location.y][location.x] = i + 1;
    if (
      location.y + move.y >= size ||
      location.y + move.y < 0 ||
      location.x + move.x >= size ||
      location.x + move.x < 0 ||
      arr[location.y + move.y][location.x + move.x]
    ) {
      if (flag) {
        move.y = move.x;
        move.x = 0;
 ...
 *
 * @example:
 *  '012345', 1 => '024135', '043215', '031425', '012345'
 *  'qwerty', 1 => 'qetwry'
...
function shuffleChar(str, iterations) {
  ...
  for (let i = 0; i < counter; i += 1) {
    ...
    if (temp === etalon && !flagInputStringEquality) {
